...
def load_video(transform: None, ranges: dict, record):
    path, indices = next(iter(ranges.items()))
    images = record.get_frames_at(indices).data

    if hasattr(transform, "tokenizer"):
        frames = transform(text="A", images=images, return_tensors="pt", padding=True)[
            "pixel_values"
        ]
    else:
...
def fixed_sampling(split, num_samples, start_frame, end_frame):
    total_frames = end_frame - start_frame + 1
    if total_frames > num_samples:
        segment_length = total_frames // num_samples

        offsets = np.arange(num_samples) * segment_length

        if split == "train":
            indices = np.random.randint(0, segment_length, size=num_samples) + offsets

        else:
            indices = offsets + segment_length // 2
    else:
        indices = np.linspace(
            0, total_frames - 1, num=num_samples, endpoint=True, dtype=int
        )
    return (indices + start_frame).astype(int)
