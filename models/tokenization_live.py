...
def get_stream_learn_ranges(
    ...
    # Ensure positive and meaningful values for frame placeholder length and number of frames
    assert (
        len_frame_placeholder_with_interval > 0
    ), "Frame placeholder interval length must be positive"
...
def build_live_tokenizer_and_update_config(
    ...
    tokenizer.add_special_tokens(
        {"additional_special_tokens": [model_config.v_placeholder, model_config.long_placeholder]}
    )
    ...
    if model_config.N_l > 0:
        # 29170
        long_placeholder_id = tokenizer.convert_tokens_to_ids(model_config.long_placeholder)
    else:
        long_placeholder_id = None
    
    model_config.update(
        dict(
            v_placeholder_id=v_placeholder_id,
            long_placeholder_id=long_placeholder_id,
...
if __name__ == "__main__":
    ...
    tokenizer = build_live_tokenizer_and_update_config(
        "meta-llama_Meta-Llama-3-8B-Instruct", config
    )
    chat = [
